...
export default function ChatBox({
  ...
  const [matriculaSelecionada, setMatriculaSelecionada] = useState<number | null>(matriculaAvaliado);
  ...
  const handleEnviar = async () => {
    if (!novaMensagem.trim() || !matriculaSelecionada || !matriculaAvaliador) {
        alert("Não foi possível enviar: dados incompletos ou usuário não identificado.");
        return;
    }
    ...
    if (res.ok) {
      ...
      if (res.status === 409) {
        ...
        alert(`Falha ao enviar comentário: ${errorData.error || 'Erro desconhecido'}`);
      }
    }
  };
